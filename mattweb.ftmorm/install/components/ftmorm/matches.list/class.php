...
class MatchesListComponent extends CBitrixComponent
{
    ...
    {        
        ...
        if(!is_null($this->arResult['DATA_COOKIE']['SORT']) && !is_null($this->arResult['DATA_COOKIE']['ORD'])){
            switch($this->arResult['DATA_COOKIE']['SORT']){
                case 'game_date':
        ...
        if($this->startResultCache(false, $cacheID)){
            ...
            // формируем фильтр для выборки на основе параметров get-запроса
            if(empty($this->arFilter) && isset($reqValues['filter'])){
                $this->arResult['FILTER_REQ_VALS'] = [];

                if($reqValues['match_city'] != 'all'){
                    $this->arFilter['GM_CITY'] = $reqValues['match_city'];
                    $this->arResult['FILTER_REQ_VALS']['match_city'] = $reqValues['match_city'];
                }
                ...
                if(!empty($reqValues['game_dstart'])){
                    ...
                    $this->arFilter['>=GM_GAME_DATE'] = new \Bitrix\Main\Type\DateTime($dtstts);
                    $this->arResult['FILTER_REQ_VALS']['game_dstart'] = $reqValues['game_dstart'];
                }
                ...
                if(!empty($reqValues['game_dfinish'])){
                    ...
                    $this->arFilter['<=GM_GAME_DATE'] = new \Bitrix\Main\Type\DateTime($dtfnts);
                    $this->arResult['FILTER_REQ_VALS']['game_dfinish'] = $reqValues['game_dfinish'];
                }
